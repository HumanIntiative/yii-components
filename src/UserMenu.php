...
class UserMenu extends CPortlet
{
    ...
    protected function renderContent()
    {
        if (Yii::app()->user->checkAccess('jagoan')) {
            $menu = array(
                ...
                array('id'=>3,'has_child'=>0,'url'=>'/proposal/index','parent_id'=>0,'title'=>'List Proposal All'),
            );
            ...
            $menu = array(
                array('id'=>1,'has_child'=>0,'url'=>'/repository/admin','parent_id'=>0,'title'=>'Repository Proposal'),
                array('id'=>2,'has_child'=>0,'url'=>'/proposal/index','parent_id'=>0,'title'=>'[List All]'),
    ...
    public function KucingMenu($items, $class="nav-second-level")
    {
        $render = '<ul class="nav '.$class.' ">';

        foreach ($items as $item) {
            $render .= '<li><a href="'.$item->url.'">' . $item->title . '</a>';
            if (!empty($item->subs)) {
                $render .= $this->KucingMenu($item->subs, 'nav-third-level');
            }
            $render .= '</li>';
        }
