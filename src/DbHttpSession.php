...
{
    public $userTableName = "sys_session_last_activity";
    public $appId = "0";
    ...
    protected function transferData($user_id, $last_activity)
    {
        ...
        $command = $db->createCommand(
            "UPDATE $this->userTableName SET last_activity=\"$last_activity\" WHERE id=$user_id AND app_id = $this->appId"
        );
        $command->execute();
        return true;
    }
    ...
    protected function clearOldSessions()
    {
        ...
        try {
            ...
            foreach ($result as $item) {
                ...
                if ($user_id > 0) {
                    $cmdUpd = $db->createCommand(
                        "UPDATE $this->userTableName SET last_activity='$last_activity', last_ip='$last_ip'  WHERE id=$user_id AND app_id = $this->appId"
                    );
                    $cmdUpd->execute();
                }
            ...
            //TODO write log
            Yii::trace('error clear 1'.$e, 'coba');
    ...
    public function writeSession($id, $data)
    {
        ...
        try {
            ...
            if ($db->createCommand()->select('id')->from($this->userTableName)->where('user_id=:user_id AND app_id=:app_id', array(':id' =>Yii::app()->getUser()->getId(), ':app_id' =>$this->appId))
                    ->queryScalar() === false
            ) {
                $db->createCommand()->insert(
                    $this->userTableName,
